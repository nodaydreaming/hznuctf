/*var problem_type_list=[{"typeId" : 1, "questionType":"web", "typeIntro":"web类型题目的简介"},
                       {"typeId" : 2, "questionType":"Pwn", "typeIntro":"Pwn类型题目的简介"},
                       {"typeId" : 3, "questionType":"逆向","typeIntro":"逆向类型题目的简介"}];*/
...
window.onload = function () {
    ...
    //fillProblemTypeList();  //测试完毕改成get_problem_type_list();!!!!!!!!!!
};

function get_problem_type_list(){
    $.ajax({
        url : '../../../Questiontype/ListQuestiontype', //获取题目类型列表
        type : 'post',
        scriptCharset : 'utf-8',
        success : function (result) {
            if(result.status == "success"){
                problem_type_list = result.typeList;
                fillProblemTypeList();
            }
        },
        error : function () {
...
function fillProblemTypeList(){
    ...
    for (var i = 0; i < problem_type_list.length; ++i){
        ...
        td4.innerText = problem_type.typeIntro == null ? "" : problem_type.typeIntro;
        td4.title = problem_type.typeIntro == null ? "" : problem_type.typeIntro;
...
function add_ProblemType(problem_type_title, problem_type_introduction){
    $.ajax({
        url : '../../../Questiontype/insert', //增加题目类型
        type : 'post',
        data :{"questionType" : problem_type_title, "typeIntro" : problem_type_introduction},
        scriptCharset : 'utf-8',
        success : function (result) {
            if(result.status == "success") {
                layer.open({
...
function editProblemType(){
    ...
    var intro = problem_type0.typeIntro == null ? "" : problem_type0.typeIntro;
    layer.open({
        type: 1,
        offset: 'auto',
        skin: 'layui-layer-lan',
...
function edit_problem_type(){
    ...
    if(problem_type_introduction == null || problem_type_introduction == ""){
        ...
        updateProblemType(problem_type.typeId, problem_type.questionType, problem_type_introduction);
    }
}

function updateProblemType(id, type, intro) {
    $.ajax({
        url : '../../../Questiontype/update', //编辑题目类型
        type : 'post',
        data : {
            "typeId" : id,
            "questionType" : type,
